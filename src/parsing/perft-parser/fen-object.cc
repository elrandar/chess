...
namespace perft_parser {
    ...
    FenObject::FenObject(std::string fenString) {
        ...
        size_t cursor = 0;
        size_t nextSlashPos;
        while ((nextSlashPos = fenString.find('/', cursor)) != std::string::npos)
        {
            ranks_.emplace_back(fenString.substr(cursor, nextSlashPos - cursor));
        ...
        side_to_move_ = fenString[cursor] == 'w'
        ...
        {
            if (fenString.at(cursor) != '-') // Check that castling ability is not empty
        ...
        {
            auto enPassantTargetFile = static_cast<board::File>(fenString.at(cursor) - 'a');
            cursor++;
            auto enPassantTargetRank = fenString.at(cursor) == '3' ? board::Rank::THREE : board::Rank::SIX;
