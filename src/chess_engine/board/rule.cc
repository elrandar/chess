...
namespace board
{
    void bitboard_to_moves(unsigned initialPosition, BitBoard pushMovesBitboard,
                           BitBoard attackMovesBitboard, PieceType pieceType,
                           Color color, std::vector<Move>& moves,
                           Chessboard_rpr& boardRpr, bool isDoublePush)
    {
        while (pushMovesBitboard != 0)
        {
            unsigned moveCell =
                BitboardOperations::bitScanForward(pushMovesBitboard);

            auto pos2 = Position(moveCell);

            auto move =
                Move(Position(initialPosition), Position(moveCell), pieceType);
            if (isDoublePush)
        ...
        {
            unsigned moveCell =
                BitboardOperations::bitScanForward(attackMovesBitboard);
            auto pos1 = Position(initialPosition);
            auto pos2 = Position(moveCell);
            auto capture = boardRpr.at(moveCell);
            if (isDoublePush)
            {
                capture = boardRpr.at(color == Color::WHITE ? moveCell - 8
                                                            : moveCell + 8);
            }
                                                      ...
                                                      BitBoard en_passant)
    {
        ...
        BitBoard enemyPieces =
            color == Color::WHITE ? boardRpr.BlackPieces : boardRpr.WhitePieces;
        ...
        while (pawns != 0)
        {
            ...
            if (color == Color::WHITE)
            {
                ...
                // Double push
                doublePushMoves =
                    (bo::nortOne(pushMoves) & bo::rank4 & eligibleSquares);
            } else
            {
                // Single push
                ...
                // Double push
                doublePushMoves =
                    (bo::soutOne(pushMoves) & eligibleSquares & bo::rank5);
            }

            attackMoves = Masks::pawn_attacks(color, pieceCell) & enemyPieces;
            auto enPassantRank = color == Color::WHITE
                ? BitboardOperations::rank6
                : BitboardOperations::rank3;
            auto enPassantattackMoves = Masks::pawn_attacks(color, pieceCell)
                & en_passant & enPassantRank;

            bitboard_to_moves(pieceCell, pushMoves, attackMoves,
                              PieceType::PAWN, color, moves, boardRpr, false);
            bitboard_to_moves(pieceCell, doublePushMoves, enPassantattackMoves,
                              PieceType::PAWN, color, moves, boardRpr, true);
            pawns &= ~(1UL << pieceCell);
        }
        return moves;
    }
    ...
    std::vector<Move>
    Rule::generate_knight_king_moves_color(PieceType pieceType, Color color,
                                           Chessboard_rpr& boardRpr,
                                           std::vector<Move>& moves)
    {
        // Get the BitBoard containing the kings
        BitBoard remainingPieces = boardRpr.get(pieceType, color);
        BitBoard eligibleSquares = color == Color::WHITE
            ? ~boardRpr.WhitePieces
            : ~boardRpr.BlackPieces;
        BitBoard enemyPieces =
            color == Color::WHITE ? boardRpr.BlackPieces : boardRpr.WhitePieces;
        ...
        while (remainingPieces != 0)
        {
            ...
            BitBoard mask = pieceType == PieceType::KING
                ? Masks::king_attacks(pieceCell)
            ...
            bitboard_to_moves(pieceCell, generatedMoves & ~enemyPieces,
                              generatedMoves & enemyPieces, pieceType, color,
                              moves, boardRpr, false);
            // Unset the bit
    ...
    std::vector<Move> Rule::generate_king_moves(Chessboard& board)
    {
        ...
        moves.reserve(9);
        if (board.isWhiteTurn())
            generate_knight_king_moves_color(PieceType::KING, Color::WHITE,
                                             boardRpr, moves);
        else
            generate_knight_king_moves_color(PieceType::KING, Color::BLACK,
                                             boardRpr, moves);
        return moves;
    }
    ...
    std::vector<Move> Rule::generate_knight_moves(Chessboard& board)
    {
        ...
        moves.reserve(8);
        if (board.isWhiteTurn())
            generate_knight_king_moves_color(PieceType::KNIGHT, Color::WHITE,
                                             boardRpr, moves);
        else
            generate_knight_king_moves_color(PieceType::KNIGHT, Color::BLACK,
                                             boardRpr, moves);
        return moves;
    }
    ...
    std::vector<Move> Rule::generate_bishop_moves(Chessboard& board)
    {
        ...
        moves.reserve(50);
        if (board.isWhiteTurn())
            generate_bishop_rook_moves_color(boardRpr, Color::WHITE,
                                             PieceType::BISHOP, moves, false);
        else
            generate_bishop_rook_moves_color(boardRpr, Color::BLACK,
                                             PieceType::BISHOP, moves, false);
        return moves;
    }

    std::vector<Move> Rule::generate_bishop_rook_moves_color(
        Chessboard_rpr& chessboardRpr, Color color, PieceType pieceType,
        std::vector<Move>& moves, bool isQueen)
    {
        BitBoard remainingPieces = isQueen
            ? chessboardRpr.get(PieceType::QUEEN, color)
            : chessboardRpr.get(pieceType, color);
        BitBoard eligibleSquares = color == Color::WHITE
            ? ~chessboardRpr.WhitePieces
            : ~chessboardRpr.BlackPieces;
        BitBoard enemyPieces = color == Color::WHITE
            ? chessboardRpr.BlackPieces
            : chessboardRpr.WhitePieces;
        ...
        while (remainingPieces != 0)
        {
            ...
            if (pieceType == PieceType::BISHOP)
                generatedMoves = magic::BishopAttacksSquare[pieceCell][index]
                    & eligibleSquares;
            else
                generatedMoves = magic::RookAttacksSquare[pieceCell][index]
                    & eligibleSquares;

            bitboard_to_moves(pieceCell, generatedMoves & ~enemyPieces,
                              generatedMoves & enemyPieces,
                              isQueen ? PieceType::QUEEN : pieceType, color,
                              moves, chessboardRpr, false);
            // Unset the bit
            remainingPieces &= ~(1UL << (pieceCell));
        }
        return moves;
    ...
    std::vector<Move> Rule::generate_queen_moves(Chessboard& board)
    {
        ...
        {
            generate_bishop_rook_moves_color(boardRpr, Color::WHITE,
                                             PieceType::ROOK, moves, true);
            generate_bishop_rook_moves_color(boardRpr, Color::WHITE,
                                             PieceType::BISHOP, moves, true);
        } else
        {
            generate_bishop_rook_moves_color(boardRpr, Color::BLACK,
                                             PieceType::ROOK, moves, true);
