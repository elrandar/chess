...
namespace board
{
    ...
    void Chessboard::do_move(Move move)
    {
        ...
        auto piece = std::pair<PieceType, Color>(
            move.piece_get(), isWhiteTurn() ? Color::WHITE : Color::BLACK);
        bool capture = move.getCapture().has_value();
        std::optional<PieceType> promotion = move.get_promotion();

        update_castling(move);

        unsigned int source_int = static_cast<int>(src.file_get())
            + static_cast<int>(src.rank_get()) * 8;
        unsigned int dest_int = static_cast<int>(dest.file_get())
            + static_cast<int>(dest.rank_get()) * 8;
        int board_index = static_cast<int>(piece.first)
            + (piece.second == Color::WHITE ? 0 : 6);

        rpr.boards.at(board_index) =
            (rpr.boards.at(board_index) & ~(1UL << source_int));
        // PROMOTION
        if (promotion.has_value())
        {
            rpr.boards.at(static_cast<int>(promotion.value())
                          + (piece.second == Color::WHITE ? 0 : 6)) |= 1UL
                << dest_int;
        } else
            rpr.boards.at(board_index) =
                (rpr.boards.at(board_index)) | 1UL << dest_int;
        // CAPTURE
        if (capture)
        {
            auto capturePiece = std::pair<PieceType, Color>(
                move.getCapture().value(),
                isWhiteTurn() ? Color::BLACK : Color::WHITE);
            int capture_board_index = static_cast<int>(capturePiece.first)
                + (capturePiece.second == Color::WHITE ? 0 : 6);
            if (move.isEnPassant())
                rpr.boards.at(capture_board_index) &=
                    ~(1UL << (dest_int + (isWhiteTurn() ? -8 : 8)));
            else
        ...
        {
            en_passant_.push(isWhiteTurn() ? 1ul << (dest_int - 8)
                                           : 1ul << (dest_int + 8));
        } else
            en_passant_.push(0ul);
        // recompute BitBoards
    ...
    void Chessboard::undo_castling(Move& move)
    {
        ...
        auto king_board = !isWhiteTurn() ? 5 : 11;
        auto rook_board = !isWhiteTurn() ? 1 : 7;
        auto king_side_rook_mask =
            !isWhiteTurn() ? ~(1UL << 5UL) : ~(1UL << 61UL);
        auto queen_side_rook_mask =
            !isWhiteTurn() ? ~(1UL << 3UL) : ~(1UL << 59UL);
        auto king_side_rook_set = !isWhiteTurn() ? 128UL : (1UL << 63UL);
        auto queen_side_rook_set = !isWhiteTurn() ? 1UL : (1UL << 56UL);

        if (move.isKingCastling())
        {
            !isWhiteTurn() ? white_king_rook_moved = 0
        ...
        {
            !isWhiteTurn() ? white_queen_rook_moved = 0
    ...
    void Chessboard::undo_move(Move move)
    {
        ...
        auto piece = std::pair<PieceType, Color>(
            move.piece_get(), isWhiteTurn() ? Color::BLACK : Color::WHITE);
        bool capture = move.getCapture().has_value();
        std::optional<PieceType> promotion = move.get_promotion();
    ...
    std::vector<Move> Chessboard::generate_legal_moves()
    {
        ...
        for (const auto& moveVector : pieceMoves)
        {
            for (auto move : moveVector)
            {
                ...
                setWhiteTurn(
                    !isWhiteTurn()); // We want to check that the king from the
                                     // color that did the move is in CHECK
                if (!is_check())
    ...
    {
        auto king_castling =
            (isWhiteTurn() ? !white_king_rook_moved && !white_king_moved
                           : !black_king_rook_moved && !black_king_moved);
        auto queen_castling =
            (isWhiteTurn() ? !white_queen_rook_moved && !white_king_moved
                           : !black_queen_rook_moved && !black_king_moved);
        ...
        auto mask_king_side_occupied =
            isWhiteTurn() ? 0x60UL : 0x6000000000000000UL;
        auto mask_queen_side_occupied =
            isWhiteTurn() ? 0xeUL : 0xe00000000000000UL;
        ...
        if (king_castling)
        {
            auto rookIsAlive = isWhiteTurn()
                ? boardRpr.get(PieceType::ROOK, Color::WHITE) & (1ul << 7)
                : boardRpr.get(PieceType::ROOK, Color::BLACK) & (1ul << 63);
            if (rookIsAlive != 0
                && (boardRpr.occupied & mask_king_side_occupied) == 0UL)
            {
                if (!is_sq_attacked_by_color(index_king_side, attacker_color)
                    && !is_sq_attacked_by_color(index_king_side + 1,
                                                attacker_color))
                {
                    auto castling =
                        Move(src_castling, dest_king_castling, PieceType::KING);
        ...
        {
            auto rookIsAlive = isWhiteTurn()
                ? boardRpr.get(PieceType::ROOK, Color::WHITE) & (1ul << 0)
                : boardRpr.get(PieceType::ROOK, Color::BLACK) & (1ul << 56);

            if (rookIsAlive != 0
                && (boardRpr.occupied & mask_queen_side_occupied) == 0UL)
            {
                if (!is_sq_attacked_by_color(index_queen_side, attacker_color)
                    && !is_sq_attacked_by_color(index_queen_side + 1,
                                                attacker_color))
                {
                    auto castling = Move(src_castling, dest_queen_castling,
                                         PieceType::KING);
    ...
    {
        BitBoard kingBoard = boardRpr.get(
            PieceType::KING, white_turn_ ? Color::WHITE : Color::BLACK);
        if (kingBoard == 0)
            return std::nullopt;
        else
    ...
    bool Chessboard::is_sq_attacked_by_color(int sq, Color color)
    {
        ...
        BitBoard bishop_queens = boardRpr.get(PieceType::BISHOP, color)
            | boardRpr.get(PieceType::QUEEN, color);
        BitBoard rook_queens = boardRpr.get(PieceType::ROOK, color)
            | boardRpr.get(PieceType::QUEEN, color);
    ...
    bool Chessboard::is_check()
    {
        ...
        if (!kingPos.has_value())
            ...
            return is_sq_attacked_by_color(
                static_cast<int>(kingPos->file_get())
                    + 8 * static_cast<int>(kingPos->rank_get()),
                isWhiteTurn() ? Color::BLACK : Color::WHITE);
    }
    ...
    Chessboard::Chessboard(perft_parser::FenObject fenObject)
    {
        ...
        if (fenObject.en_passant_target_get().has_value())
        {
            ...
            uint8_t enPassantSquare =
                static_cast<int>(enPassantPos.rank_get()) * 8
